...
#include <sys/stat.h> //image size for mmap
#include <string.h>
#include <unistd.h>
#include <vector>
#include <math.h>       /* pow */
...
void *map;
int group_count, source_node_group, dest_node_group;
struct stat img_stat;
vector<ext2_super_block*> supers;
vector<ext2_block_group_descriptor*> bgds;  // such a master piece !
ext2_inode* root;
void *src_refmap, *dest_refmap;
void *src_bitmap, *dest_inodemap;
vector<ext2_dir_entry*> dir_entries;
vector<uint32_t> group_initial_blocks;
vector<string> sourcePath;
vector<string> destinationPath;
string newFileName;
